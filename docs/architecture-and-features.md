...
| **CRITICAL** | External BI database connection strings stored in plaintext (no encryption at rest) <br>`migrations/024_bi_database_connections.up.sql:6` | bi_database_connections.connection_string is TEXT (migration 024) and written/read verbatim; mask_connection_string only masks API responses, not the at-rest connection string. |
...
_Today:_ Pulse's "AI" is pure keyword heuristics with no external LLM provider calls wired anywhere (no OpenAI/Anthropic/Gemini client deps in `crates/pulse-server/Cargo.toml` and no LLM-related env vars in `crates/pulse-server/src/config.rs`). In services/ai.rs, answer_query (lines 193-337) calls detect_intent (lines 880-900), which is substring matching ("error"/"page"/"referrer"/"event"/"device"/"country"/"trend").
